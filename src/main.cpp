...
void setup() {
  ...
  if (debug) tft.print("LED output OK");
  ...
  if (debug) tft.println("VS1053 found");


  if (!SD.begin(CARDCS)) {
    Serial.println(F("SD failed, or not present"));
    if (debug) tft.print(" / SD fail");
    while (1)
      ;  // don't do anything more
  }
  ...
  if (!SDfs.begin(CARDCS)) {  // Breakouts require 10 MHz limit due to longer wires
...
void loop() {
  ...
  if (browsermode) {
    ...
    if (sLp) {  // if in browser mode, stick left displays the previous book in the series
    ...
    if (sRp) {  // if in browser mode, stick left displays the next book in the series
    ...
    if (sUp) {  // if in browser mode, stick up displays the first book in the previous series
      bookIndB = 1;
      if (seriesIndB - 1 > 0) {
    ...
    if (sDp) {  // if in browser mode, stick left displays the first book in the next series
      bookIndB = 1;
    ...
    if (browsercounter > browserOutCount) {  // if going out of browser mode, display back the current playing image and track number
      displayPlaying();
      browsermode = false;
  ...
  // skip forward
  if (bAp && screenOn) {
    if (trackInd < numTracks) { // numTracksCount(seriesInd, bookInd)) {
      trackInd = trackInd + 1;
      PlayTrack(seriesInd, bookInd, trackInd);
    } else {
      // blink red LED
